 ...
 struct Student{
     ...
     char exit_date[11];  
     float deposit;
     int rent_status[NUM_MONTHS];             
     int electricity_bill_status[NUM_MONTHS]; 
     float electricity_bill_amount[NUM_MONTHS];
 };

 // Menu 1 ogrenci ekleme fonksiyonu
 void addStudent(){
     ...
     // Ayni tcyi var mı diye kontrol eden döngü
     int tcExists = 0;
 ...
 void deleteStudent(){
     ...
     // Dosyadan öğrenci bilgilerini oku ve geçici bir dosyaya yaz
     FILE *fp = fopen("students.txt", "r");
     FILE *temp_fp = fopen("temp.txt", "w");
     char line[100];
     int idFound = 0;
     while (fgets(line, 100, fp) != NULL){
         // Öğrenci bilgilerini bölme
         struct Student student;
         sscanf(line, "TC: %s Id: %s", student.tc, student.id);
         if (strcmp(student.id, id) == 1){
             // Eşleşen id bulundu, öğrenci bilgilerini geçici dosyaya yazmayınız.
             idFound = 1;
         }
         else{
             // Eşleşen kimlik bulunamadı, öğrencinin bilgilerini geçici dosyaya yazın
             fprintf(temp_fp, "TC: %s Id: %s", student.tc, student.id);
     ...
     // Geçici dosyayı orijinal dosya olarak kullanın
     remove("students.txt");
